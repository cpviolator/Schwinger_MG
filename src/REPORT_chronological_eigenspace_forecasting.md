...
6. **FEAST eigensolver**: The FEAST algorithm (Polizzi, 2009) computes eigenvalues
   within a specified spectral interval using contour integration in the complex
   plane. This is a natural fit for the Chebyshev-window tracking (Strategy D):
   - FEAST targets eigenvalues in [a, b] directly, rather than finding the k
     smallest and hoping they fall in the window
   - The contour integral acts as a spectral projector, automatically capturing
     new eigenmodes that enter the window as the gauge evolves
   - FEAST can be warm-started with previous eigenvectors as the initial subspace,
     combining naturally with the generator forecasting (Strategy B)
   - The forecasted rotation pre-rotates the FEAST subspace, giving a better
     starting point for the contour integration
   - For the coarse operator (dim=256, k=8-16), FEAST's cost is dominated by
     the contour quadrature points (typically 8-16 shifted linear solves), which
     can be parallelised. Each solve is on the coarse grid so is cheap.
   - Key advantage over TRLM: FEAST naturally handles eigenvalue crossings at
     the window boundary — modes entering or leaving the interval are
     automatically included/excluded without special treatment
   - Consider implementing FEAST as an alternative to TRLM for the periodic
     fresh eigensolver, with the forecasted subspace as the initial guess
